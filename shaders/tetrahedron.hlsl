...
float DistanceEstimator(float3 p) {
    p.yz = mul(Rotate(0.20 * PI), p.yz);
    p.yx = mul(Rotate(0.25 * PI), p.yx);
...
float4 RayMarcher(float3 ro, float3 rd) {
    ...
    for (steps = 0.0; steps < float(MaximumRaySteps); steps++) {
        float3 p = ro + totalDistance * rd;      // Current position of the ray
    ...
    if (hit) {
        col.rgb = float3(0.8 + (length(curPos) / 8.5), 1.0, 0.8);
...
void mainImage(out float4 fragColor, in float2 fragCoord) {
    ...
    ro.yz = mul(Rotate(-m.y * 2.0 * PI + PI - 1.1), ro.yz); // Rotate thew ray with the mouse rotation
    ro.xz = mul(Rotate(-iTime * 2.0 * PI / 10.0), ro.xz);
